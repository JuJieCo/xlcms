...
$(document).ready(function() {
		...
		oTable.on('click', function(e) {
			...
			eEle.find('input').eq(1).attr("name","rkjlList["+num+"].kuCun.dqsl");
			eEle.find('input').eq(1).attr("id","dqsl"+num);
			eEle.find('input').eq(2).attr("name","rkjlList["+num+"].bcsl");
			//eEle.find('input').eq(2).attr("id","bcsl"+num);
			eEle.find('input').eq(3).attr("name","rkjlList["+num+"].danjia");
			//eEle.find('input').eq(3).attr("id","danjia"+num);
			...
			eEle.find('input').eq(4).attr("name","rkjlList["+num+"].kuCun.remark");
			eEle.find('input').eq(5).attr("name","rkjlList["+num+"].kuCun.kcid");
			eEle.find('input').eq(5).attr("id","kcid"+num);
			//alert(eEle.find('input').eq(0).attr("id"));
			oTable.append(eEle);
			
			
			$("#wlmc").autocomplete("${pageContext.request.contextPath}/autoCompleteAction!showKuCunTYListAuto", {
				minChars : 1,
				max : 50,
				width : 500,
				matchContains : true,
			...
			$("#wlmc"+num).autocomplete("${pageContext.request.contextPath}/autoCompleteAction!showKuCunTYListAuto", {
				minChars : 1,
				max : 50,
				width : 500,
				matchContains : true,
