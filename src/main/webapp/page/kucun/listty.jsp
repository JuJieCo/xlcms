...
<body>
		...
		<div class="right" id="mainFrame">
			<div class="right_cont">
				<ul class="breadcrumb"> <li>当前位置：首页<span class="divider">/</span>库存管理 <span class="divider">/</span>查询通用配件库存</li></ul>
				<div class="title_right">
					<strong>查询通用配件库存</strong>
				</div>
				
				<div style="width: 95%; margin: auto">
				<form action="${pageContext.request.contextPath}/kuCunAction!showKuCunTYList" name="form1"  method="post" >
				<s:token name="s_token"></s:token>
						...
						<tr>
							...
							<td width="10%" >
								<select name="kuCun.wllx"  class="span1-1">
									...
									<option value="1" ${kuCun.wllx==1?"selected":"" }>油水</option>
									<option value="2" ${kuCun.wllx==2?"selected":"" }>滤芯</option>
									<option value="3" ${kuCun.wllx==3?"selected":"" }>清洗剂</option>
									<option value="4" ${kuCun.wllx==4?"selected":"" }>刹车片</option>
									<option value="5" ${kuCun.wllx==5?"selected":"" }>火花塞</option>
									<option value="6" ${kuCun.wllx==6?"selected":"" }>配件</option>
									<option value="7" ${kuCun.wllx==7?"selected":"" }>辅料</option>
									<option value="8" ${kuCun.wllx==8?"selected":"" }>油漆</option>
									<option value="9" ${kuCun.wllx==9?"selected":"" }>玻璃</option>
					...
					<table class="margin-bottom-20 table  no-border">
						<tr>
							...
							<td class="text-center"><input type="button" value="新建"  onclick="location.href='${pageContext.request.contextPath}/kuCunAction!showKuCunTYForModifyAsSave'"  id="modal-9735581" class="btn btn-info btn-small" style="width: 80px;" /></td>
					...
					<table class="table table-bordered">
						<tr>
							...
							<td align="center" nowrap="nowrap" bgcolor="#f1f1f1"><strong>库存数量</strong></td>
						...
						<tr>
							<td align="center">&nbsp;${list.wlmc }</td>
						 	<td align="center">&nbsp;
								<c:if test="${list.wllx eq 1 }">油水</c:if>
						 		<c:if test="${list.wllx eq 2 }">滤芯</c:if>
						 		<c:if test="${list.wllx eq 3 }">清洗剂</c:if>
						 		<c:if test="${list.wllx eq 4 }">刹车片</c:if>
						 		<c:if test="${list.wllx eq 5 }">火花塞</c:if>
						 		<c:if test="${list.wllx eq 6 }">配件</c:if>
						 		<c:if test="${list.wllx eq 7 }">辅料</c:if>
						 		<c:if test="${list.wllx eq 8 }">油漆</c:if>
						 		<c:if test="${list.wllx eq 9 }">玻璃</c:if>
							...
							<td align="center">		
								<a href="${pageContext.request.contextPath}/kuCunAction!showJiluList?kcid=${list.kcid}">查看记录</a>&nbsp;&nbsp;
							</td>
						</tr>
						</s:iterator>
					</table>
