...
function check(){
	for(var i=0 ; i<'${baoJiaList.size()}'; i++){ 
		if($("#bjzt"+i).val()==3&&$("#duiliao"+i).val()!=""){
			 if($("#duiliao"+i).val()!=$("#wlmc"+i).val()){
		...
		if($("#bjzt"+i).val()==3&&$("#duiliao"+i).val()!=""){
			 if(parseInt($("#shuliang"+i).val())>parseInt($("#dqsl"+i).val())){
...
<body>
 ...
 <%@include file="/page/plugin/autocomplete_beiliao.jsp"%>
	<div class="right" id="mainFrame">
	<div class="right_cont">
		<ul class="breadcrumb"> <li>当前位置：首页<span class="divider">/</span>报价管理 <span class="divider">/</span>创建备料单</li></ul>
			<div class="title_right">
				<strong>创建备料单</strong>
			</div>
				
<form name="form" method="post" action="${pageContext.request.contextPath}/baoJiaAction!modifyBeiLiaoForSave?gdid=${shiGongDan.gdid}"   onsubmit="return validateForm(this)">
		...
		<table class="table table-bordered">
			...
			<table class="table table-bordered" id="count">
				<tr>
					...
					<td align="center" nowrap="nowrap" bgcolor="#f1f1f1"><strong>库存对料</strong></td>
				...
				<tr>
					<td align="center">&nbsp;
						...
						<input type="hidden" name="baoJiaList[<s:property value='#st.index'/>].sfyh"  id="sfyh<s:property value='#st.index'/>"  class="span1" value="${list.sfyh}"/>
						...
						<input type="hidden" name=""  id="dqsl<s:property value='#st.index'/>"  class="span1" />
					</td>
					<td align="center">&nbsp;
					...
					<td align="center">&nbsp;
						...
						<c:if test="${list.bjzt eq 3 && list.sfyh eq 1}">
						<input type="text" name=""  id="duiliao<s:property value='#st.index'/>"  class="span1" value=""   onblur="check();"/>
						</c:if> 
					</td>
					<td align="center">&nbsp;${list.bjrq }</td>
					<td align="center">&nbsp;${list.fhrq }</td>
					<td align="center">&nbsp;
						<c:if test="${list.sfyh eq 1}">
						<input type="text" name="baoJiaList[<s:property value='#st.index'/>].rkrq"  id="rkrq<s:property value='#st.index'/>"  class="span1-1" value="${list.rkrq}" readonly="readonly"/>
						</c:if>
					</td>
					...
					<td align="center">&nbsp;
						<a href="${pageContext.request.contextPath}/baoJiaAction!showWlmcByID?bjid=${list.bjid}&gdid=${shiGongDan.gdid}">修改物料名称</a>&nbsp;&nbsp;
						 <a href="${pageContext.request.contextPath}/baoJiaAction!modifyBaoJiaForDelete?tag=beiliao&bjid=${list.bjid }&gdid=${shiGongDan.gdid}" onclick="return(confirm('确认要删除吗?'));">删除</a>&nbsp;&nbsp;  
					</td>
				</tr>
				</s:iterator>
			</table>
